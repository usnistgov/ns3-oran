                        ...
                        uint16_t rnti)
{
  ...
  Config::Disconnect ("NodeList/*/DeviceList/*/$ns3::LteNetDevice/$ns3::LteEnbNetDevice/LteEnbRrc/UeMap/*/DataRadioBearerMap/*/LteRlc/RxPDU",
      MakeCallback (&TraceEnbRx));

  Config::Connect ("NodeList/*/DeviceList/*/$ns3::LteNetDevice/$ns3::LteEnbNetDevice/LteEnbRrc/UeMap/*/DataRadioBearerMap/*/LteRlc/RxPDU",
      MakeCallback (&TraceEnbRx));
}
...
RxTrace (std::string context, Ptr< const Packet > packet, const Address &address)
{
  ...
  std::cout << Simulator::Now ().GetSeconds () << " s: " << context
    << " recieved " << packet->GetSize ()
    << " bytes from " << isa.GetIpv4 ()
    << " on port " << (uint32_t)isa.GetPort ()
    << std::endl;
}
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("application-output", "Enable printing application traffic information", applicationOutput);
      ...
      for (uint32_t netDevIdx = 1; netDevIdx < ue->GetNDevices (); netDevIdx++)
        {
          ...
          ueStaticRouting->AddHostRouteTo (remoteHostAddr, epcHelper->GetUeDefaultGatewayAddress (), netDevIdx);

          dlPort++;

          OnOffHelper dlClientHelper ("ns3::UdpSocketFactory", InetSocketAddress (ueAddress, dlPort));
          dlClientHelper.SetAttribute ("Local", AddressValue (InetSocketAddress (remoteHostAddr, dlPort)));
          ...
          ulClientHelper.SetAttribute ("Local", AddressValue (InetSocketAddress (ueAddress, ulPort)));
  ...
  oranHelper->SetAttribute ("LmQueryMaxWaitTime", TimeValue (Seconds (0))); // 0 means wait for all LMs to finish
  oranHelper->SetAttribute ("LmQueryLateCommandPolicy", EnumValue (OranNearRtRic::DROP));
  oranHelper->SetAttribute ("RicTransmissionDelayRv", StringValue ("ns3::ConstantRandomVariable[Constant=0.001]"));
  ...
  oranHelper->SetDefaultLogicModule ("ns3::OranLmLte2LteDistanceHandover",
      "ProcessingDelayRv", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
  ...
  for (uint32_t netDevIdx = 0; netDevIdx < numberOfNetDevs; netDevIdx++)
    {
      oranHelper->SetE2NodeTerminator ("ns3::OranE2NodeTerminatorLteUe",
          ...
          "TransmissionDelayRv", StringValue ("ns3::ConstantRandomVariable[Constant=0.001]"));
  ...
  if (enbOutput)
    {
      ...
      Simulator::Schedule (Seconds (0.5),
          &Config::Connect,
          "NodeList/*/DeviceList/*/$ns3::LteNetDevice/$ns3::LteEnbNetDevice/LteEnbRrc/UeMap/*/DataRadioBearerMap/*/LteRlc/RxPDU",
          MakeCallback (&TraceEnbRx));
    }
