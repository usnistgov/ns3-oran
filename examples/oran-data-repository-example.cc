...
int main (int argc, char* argv[])
{
  ...
  std::map<Time, Vector> lteUeNodePositions = repository->GetNodePositions (lteUeE2NodeId, Seconds (0), Seconds (15));

  // Report the retrieved positions.
  for (auto it = lteUeNodePositions.begin (); it != lteUeNodePositions.end (); it++)
    {
      std::cout << "E2 Node with ID = " << lteUeE2NodeId
        << " located at (" << it->second.x
          << ", " << it->second.y
          << ", " << it->second.z
        << ")"
        << " at time t = " << it->first.As (Time::S)
        << std::endl;
    }
  ...
  if (found)
    {
      std::cout << "LTE UE with E2 Node ID = " << lteUeE2NodeId
        ...
        << " is connected to cell with Cell ID = " << (uint32_t)cellId
        << std::endl; 
    }
