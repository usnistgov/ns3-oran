...
{
  static TypeId tid = TypeId ("ns3::OranHelper")
    .SetParent<Object> ()
    ...
    .AddAttribute ("E2NodeInactivityThreshold",
                   "The amount of time since an E2 node's last registration request before deregistration.",
                   TimeValue (Seconds (2)),
...
OranHelper::CreateNearRtRic (void)
{
  ...
  nearRtRicE2Terminator->SetAttribute ("TransmissionDelayRv", PointerValue (m_ricTransmissionDelayRv));
  ...
  nearRtRic->SetAttribute ("E2NodeInactivityIntervalRv", PointerValue (m_e2NodeInactivityIntervalRv));
  ...
  for (auto queryTriggerFactory : m_queryTriggerFactories)
    {
      ...
      if (!nearRtRic->AddQueryTrigger (queryTriggerFactory.first, queryTrigger))
        {
          NS_ABORT_MSG ("Could not add the \"" << queryTriggerFactory.first << "\" query trigger to Near-RT RIC.");
        }
    }
...
OranHelper::DeployTerminators (Ptr<OranNearRtRic> nearRtRic, NodeContainer nodes, uint32_t netDeviceIndex)
{
  NS_LOG_FUNCTION (this);

  OranE2NodeTerminatorContainer terminators;

  NS_ABORT_MSG_IF (nearRtRic == nullptr, "Attempting to deploy E2 Node Terminators with a null Near-RT RIC");

  for (auto nodeIt = nodes.Begin (); nodeIt != nodes.End (); nodeIt++)
    {
      Ptr<OranE2NodeTerminator> terminator = m_e2NodeTerminatorFactory.Create<OranE2NodeTerminator> ();
