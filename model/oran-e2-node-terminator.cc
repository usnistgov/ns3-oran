...
{
  static TypeId tid = TypeId ("ns3::OranE2NodeTerminator")
    .SetParent<Object> ()
    ...
    .AddAttribute ("TransmissionDelayRv",
                   "The random variable used (in seconds) to calculate the transmission delay for a report.",
  ...
  if (m_active)
    {
      ...
      if (registrationDelay > e2NodeInactivityThresholdVal.Get ())
        {
          NS_LOG_WARN ("E2 Node Terminator registration delay is larger than Near-RT RIC inactivity threshold.");
        }

      m_registrationEvent = Simulator::Schedule (
          registrationDelay,
  ...
  if (m_active)
    {
      m_sendEvent = Simulator::Schedule (
          Seconds (m_sendIntervalRv->GetValue ()), 
