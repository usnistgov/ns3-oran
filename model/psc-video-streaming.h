...
private:
  ...
  /**
   * Sends the next video codec packet. If the packet size exceeds m_maxUdpDatagramSize bytes, the
  ...
  /**
   * Maximum size of generated UDP datagrams. This value has to be greater
   ...
   *
   * By default this value is 1500 Bytes, to mimic the behavior observed in
   * ffmpeg for RTP streams.
   */
  uint16_t m_maxUdpDatagramSize;
  ...
  /**
   * Map with the data distribution for the packet intervals
   * These values have been loaded from the source (built-in distributions
   * or custom files), but are not used until LoadDistributions () is called
   */
