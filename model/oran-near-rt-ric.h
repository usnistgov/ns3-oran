...
public:
   ...
   */
  void NotifyLmFinished (Time lmQueryCycle, std::vector<Ptr<OranCommand> > commands, Ptr<OranLm> lm);
  /**
...
private:
  ...
  /**
   * The amount of time that must pass from a node's last registration before determining a node is inactive.
   */
