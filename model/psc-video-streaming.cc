...
PscVideoStreaming::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PscVideoStreaming")
    ...
    .AddAttribute ("MaxUdpDatagramSize",
                   "Maximum size of the generated UDP datagrams",
                   UintegerValue (1500),
                   MakeUintegerAccessor (&PscVideoStreaming::m_maxUdpDatagramSize),
    ...
    .AddTraceSource ("TxWithAddresses", "A new packet is created and is sent",
                      MakeTraceSourceAccessor (&PscVideoStreaming::m_txTraceWithAddresses),
...
PscVideoStreaming::LoadCdfs (void)
{
  ...
  for (std::map<double, uint32_t>::iterator it = m_sizeDistribution.begin (); it != m_sizeDistribution.end (); it++)
    {
  ...
  for (std::map<double, double>::iterator it = m_intervalDistribution.begin (); it != m_intervalDistribution.end (); it++)
    {
  ...
  if (Simulator::Now () < m_stopTime && m_socket)
    {
      ...
      while (pending > 0)
        {
          if (pending < m_maxUdpDatagramSize)
            {
              p = Create <Packet> (pending);
            }
          else
            {
              p = Create <Packet> (m_maxUdpDatagramSize); 
            }
          ...
          m_txTraceWithAddresses (p, localAddress, InetSocketAddress::ConvertFrom (InetSocketAddress (Ipv4Address::ConvertFrom (m_receiverAddress), m_receiverPort)));
