...
{
  static TypeId tid = TypeId ("ns3::OranDataRepositorySqlite")
    .SetParent<OranDataRepository> ()
    .AddConstructor<OranDataRepositorySqlite> ()
    .AddAttribute ("DatabaseFile", "The database file path.",
                   StringValue ("oran-repository.db"),
                   MakeStringAccessor (&OranDataRepositorySqlite::m_dbPath),
                   MakeStringChecker ())
    .AddTraceSource ("QueryRc", "Return code for SQL queries",
  ...
  if (m_active)
    {
      ...
      sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[INSERT_NODE_REGISTRATION].c_str (), -1, &stmt, 0);
      ...
      CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (e2NodeId, true, Simulator::Now ().GetTimeStep ()));
  ...
  if (m_active)
    {
      ...
      rc = sqlite3_step (stmt);
      CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (e2NodeId, false, Simulator::Now ().GetTimeStep ()));
      sqlite3_finalize (stmt);
    }
  return retVal;
}
      ...
      if (IsNodeRegistered (e2NodeId))
        {
          ...
          sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[INSERT_NODE_LOCATION].c_str (), -1, &stmt, 0);
          ...
          rc = sqlite3_step (stmt);
          CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (e2NodeId, pos.x, pos.y, pos.z, t.GetTimeStep ()));
          sqlite3_finalize (stmt);
        }
    }
}
...
void
OranDataRepositorySqlite::SaveLteUeCellInfo (uint64_t e2NodeId, uint16_t cellId, uint16_t rnti, Time t)
{
      ...
      if (IsNodeRegistered (e2NodeId))
        {
          ...
          rc = sqlite3_step (stmt);
          CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (e2NodeId, cellId, rnti, t.GetTimeStep ()));
          sqlite3_finalize (stmt);
        }
    }
}
...
std::map<Time, Vector>
OranDataRepositorySqlite::GetNodePositions (uint64_t e2NodeId, Time fromTime, Time toTime, uint64_t maxEntries)
{
      ...
      if (IsNodeRegistered (e2NodeId))
        {
          ...
          sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_NODE_ALL_POSITIONS].c_str (), -1, &stmt, 0);
      ...
      if (IsNodeRegistered (e2NodeId))
        {
          ...
          sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_LTE_UE_CELLINFO].c_str (), -1, &stmt, 0);
          sqlite3_bind_int64 (stmt, 1, e2NodeId);
  ...
  if (m_active)
    {
      ...
      if (sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_LTE_ALL_UE_E2NODEIDS].c_str (), -1, &stmt, 0) != SQLITE_OK) {
...
OranDataRepositorySqlite::GetLteUeE2NodeIdFromCellInfo (uint16_t cellId, uint16_t rnti)
{
  ...
  if (m_active)
    { 
      ...
      sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_LTE_UE_E2NODEID_FROM_CELLINFO].c_str (), -1, &stmt, 0);
      ...
      if (IsNodeRegistered (e2NodeId))
        {
          ...
          sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_LTE_CELLID_FROM_E2NODEID].c_str (), -1, &stmt, 0);
          sqlite3_bind_int64 (stmt, 1, e2NodeId);
  ...
  if (m_active)
    {
      ...
      sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_LTE_ALL_ENB_E2NODEIDS].c_str (), -1, &stmt, 0);
  ...
  if (m_active) 
    {
      ...
      sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[GET_ALL_LAST_REGISTRATION_TIMES].c_str (), -1, &stmt, 0);
      ...
      if (IsNodeRegistered (cmd->GetTargetE2NodeId ()))
        {
          ...
          sqlite3_prepare_v2 (m_db, m_queryStmtsStrings[LOG_E2TERMINATOR_COMMAND].c_str (), -1, &stmt, 0);
          ...
          rc = sqlite3_step (stmt);
          CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (cmd->GetTargetE2NodeId (), Simulator::Now ().GetTimeStep (), cmd->ToString ()));
          sqlite3_finalize (stmt);
        }
    }
}
  ...
  if (m_active)
    {
      ...
      rc = sqlite3_step (stmt);
      CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (lm, Simulator::Now ().GetTimeStep (), cmd->ToString ()));
      sqlite3_finalize (stmt);
    }
}
  ...
  if (m_active)
    {
      ...
      CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (lm, Simulator::Now ().GetTimeStep (), logStr));
      sqlite3_finalize (stmt);
    }
}
  ...
  if (m_active)
    {
      ...
      CheckQueryReturnCode (stmt, rc, FormatBoundArgsList (cmm, Simulator::Now ().GetTimeStep (), logStr));
      sqlite3_finalize (stmt);
    }
}

void
OranDataRepositorySqlite::CheckQueryReturnCode (sqlite3_stmt* stmt, int rc, std::string boundParmsStr) const
{
  ...
  if (m_dbPath == ":memory:")
    {
      ...
      std::cerr << "WARNING: Using in-memory DB for the ORAN Storage. DB will not be saved to disk." << std::endl;
    }
  else
    {
      ...
      if (m_dbPath.find (":") != std::string::npos) 
        {
          NS_ABORT_MSG ("File name for the ORAN Storage DB (" << m_dbPath << ") is an URI. URI-named DBs are not supported");
        }
    }

  if (m_dbPath.empty ())
    {
      NS_LOG_WARN ("Using a randomly named temporary file as DB for the ORAN Storage. DB will not be available after simulation ends.");
      std::cerr << "WARNING: Using a randomly named temporary file as DB for the ORAN Storage. DB will not be available after simulation ends." << std::endl;
    }
...
OranDataRepositorySqlite::InitStatements (void)
{
  ...
  m_createStmtsStrings [INDEX_NODE_REGISTRATION] = "CREATE INDEX IF NOT EXISTS "
    "idx_noderegistration_nodeid ON noderegistration(nodeid);";

  m_createStmtsStrings [TABLE_CMM_ACTION] = "CREATE TABLE IF NOT EXISTS cmmaction ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "cmmname        TEXT                              NOT NULL, "
    "simulationtime INTEGER                           NOT NULL, "
    "description    TEXT                              NOT NULL);";

  m_createStmtsStrings [TABLE_LM_ACTION] = "CREATE TABLE IF NOT EXISTS lmaction ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "lmname         TEXT                              NOT NULL, "
    "simulationtime INTEGER                           NOT NULL, "
    "description    TEXT                              NOT NULL);";

  m_createStmtsStrings [TABLE_LM_COMMAND] = "CREATE TABLE IF NOT EXISTS lmcommand ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "lmname         TEXT                              NOT NULL, "
    "simulationtime INTEGER                           NOT NULL, "
  ...
  m_createStmtsStrings [TABLE_LTE_UE_CELL] = "CREATE TABLE IF NOT EXISTS lteuecell ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
  ...
  m_createStmtsStrings [TABLE_NODE] = "CREATE TABLE IF NOT EXISTS node ("
    "nodeid         INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "nodetype       INTEGER                           NOT NULL);";


  m_createStmtsStrings [TABLE_NODE_LOCATION] = "CREATE TABLE IF NOT EXISTS nodelocation ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "nodeid         INTEGER                           NOT NULL, "
    "x              REAL                              NOT NULL, "
    "y              REAL                              NOT NULL, "
  ...
  m_createStmtsStrings [TABLE_NODE_REGISTRATION] = "CREATE TABLE IF NOT EXISTS noderegistration ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "nodeid         INTEGER                           NOT NULL, "
  ...
  m_createStmtsStrings [TABLE_TERMINATOR_COMMAND] = "CREATE TABLE IF NOT EXISTS terminatorcommand ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
  ...
  m_createStmtsStrings[TABLE_APPLOSS_COMMAND] = "CREATE TABLE IF NOT EXISTS nodeapploss ("
    "entryid        INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
    "nodeid         INTEGER                           NOT NULL, "
    "loss           REAL                              NOT NULL, "
    "simulationtime INTEGER                           NOT NULL, "
  ...
  m_queryStmtsStrings [GET_LTE_ALL_ENB_E2NODEIDS] = "SELECT nr.nodeid, MAX(nr.simulationtime) "
  ...
  m_queryStmtsStrings [GET_NODE_ALL_POSITIONS] = "SELECT simulationtime, x, y, z "
    "FROM nodelocation "
  ...
  m_queryStmtsStrings [INSERT_LTE_UE_CELL] = "INSERT INTO lteuecell "
  ...
  m_queryStmtsStrings [INSERT_NODE_LOCATION] = "INSERT INTO nodelocation "
    "(nodeid, x, y, z, simulationtime) VALUES (?, ?, ?, ?, ?);";

  m_queryStmtsStrings [INSERT_NODE_REGISTRATION] = "INSERT INTO noderegistration "
    "(nodeid, registered, simulationtime) VALUES (?, ?, ?);";

  m_queryStmtsStrings [LOG_CMM_ACTION] = "INSERT INTO cmmaction "
    "(cmmname, simulationtime, description) VALUES (?, ?, ?);";

  m_queryStmtsStrings [LOG_E2TERMINATOR_COMMAND] = "INSERT INTO terminatorcommand "
