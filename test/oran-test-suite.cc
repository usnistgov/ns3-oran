...
OranTestCaseMobility1::DoRun (void)
{
  ...
  Ptr<ConstantVelocityMobilityModel> mobility = nodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ();
  ...
  Simulator::Schedule (Seconds (1), &OranHelper::ActivateE2NodeTerminators, oranHelper, e2NodeTerminators);
  Simulator::Schedule (Seconds (2), &ConstantVelocityMobilityModel::SetVelocity, mobility, Vector (speed, speed, 0));
  Simulator::Schedule (Seconds (12), &ConstantVelocityMobilityModel::SetVelocity, mobility, Vector (0, 0, 0));
  // ORAN Models -- END

  Simulator::Stop (simTime);
  Simulator::Run ();

  std::map<Time, Vector> nodePositions = nearRtRic->Data ()->GetNodePositions (1, Seconds (0), simTime, 12);
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (firstPosition.x, 0.0, 0.001, "First position x-coordinate does not match.");
  NS_TEST_ASSERT_MSG_EQ_TOL (firstPosition.y, 0.0, 0.001, "First position y-coordinate does not match.");
  NS_TEST_ASSERT_MSG_EQ_TOL (firstPosition.z, 0.0, 0.001, "First position z-coordinate does not match.");

  // Check the node's last reported position.
  NS_TEST_ASSERT_MSG_EQ_TOL (lastPosition.x, 20.0, 0.001, "Last position x-coordinate does not match.");
  NS_TEST_ASSERT_MSG_EQ_TOL (lastPosition.y, 20.0, 0.001, "Last position y-coordinate does not match.");
  NS_TEST_ASSERT_MSG_EQ_TOL (lastPosition.z, 0.0, 0.001, "Last position z-coordinate does not match.");
